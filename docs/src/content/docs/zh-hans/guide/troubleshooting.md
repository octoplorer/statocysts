...
| 失败时机                                            | 常见原因                                             | 错误结构                    |
| --------------------------------------------------- | ---------------------------------------------------- | --------------------------- |
| `createNotifier()` 返回时                           | 列表为空、URL/目标重复/协议错误、提供方格式无效      | 同步目标或提供方错误        |
| 顶层 `send()` 在投递前拒绝时                        | 与 `createNotifier()` 相同的本地目标或提供方校验错误 | 原始目标或提供方错误        |
| 顶层 `send()` 或通知器 `.send()` 在投递开始后拒绝时 | 请求准备或传输失败                                   | `NotificationDeliveryError` |
| 具名提供方 `validate()` 抛出时                      | 目标 URL 或提供方格式无效                            | 原始目标或提供方错误        |
| 具名提供方 `.send()` 拒绝时                         | 提供方校验、请求准备或传输失败                       | 原始提供方或传输错误        |
...
4. 至少提供了一个通知目标。
5. 提供方要求的凭据、路径段和查询值均存在且通过本地规则。

可以使用 CLI 执行不会联系远程提供方的运行时及提供方本地校验：

```sh
stato verify -u "$NOTIFICATION_TARGET"
```

`verify` 会校验提供方专属路径段、凭据和接收者的格式及必填状态，但不会确认这些值是否真实存在，也不会测试远程连通性。
...
如果运行时校验失败，请将通知目标与对应提供方参考页面进行比较。本地校验会发现以下常见问题：
...
2. 运行 `stato verify` 隔离本地运行时和提供方 URL 错误。
