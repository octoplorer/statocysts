...
系统 SHALL 以 `qqbot:` 协议注册一个新的 chat provider，遵循 `defineProvider` 约定的 provider 接口。

#### Scenario: Provider 在 registry 中可用

- **WHEN** 用户通过 `createSender` 或 `send` 使用 `qqbot:` 协议的 URL
- **THEN** 系统找到并调用 qqbot provider 处理该 URL
...
系统 SHALL 在浏览器入口（`browser.ts`）中注册 qqbot provider，因为其仅依赖 HTTP transport。

#### Scenario: 浏览器入口导出 qqbot

- **WHEN** 从 `statocysts/browser` 导入
- **THEN** `qqbot` 在导出列表中可用，`senderRegistry` 能识别 `qqbot:` 协议
