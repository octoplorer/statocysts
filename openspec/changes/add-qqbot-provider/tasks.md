...
- [ ] 1.1 创建 `packages/statocysts/src/services/chat/qqbot/index.ts` 实现 qqbot provider
...
- [ ] 2.1 创建 `packages/statocysts/src/services/chat/qqbot/index.spec.ts`
...
- [ ] 3.1 在 `packages/statocysts/src/index.ts` 中导入并注册 `qqbot`
- [ ] 3.2 在 `packages/statocysts/src/browser.ts` 中导入并注册 `qqbot`
- [ ] 3.3 在 `packages/statocysts/src/index.ts` 的 export 中添加 `qqbot`

## 4. 验证

- [ ] 4.1 运行 `pnpm test` 确保所有测试通过
- [ ] 4.2 运行 `pnpm lint` 确保代码风格一致
